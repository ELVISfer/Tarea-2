...
app.get("/noticias", (res) => {
  res.json(noticias);
});

// Metodo GET
app.get("/noticia", (res) => {
  fs.readFile(path.join(__dirname, "main.html"), (contenidoArchivo) => {
...
app.delete("/noticia/:id", (req, res) => {
  const { id } = req.params;
  const indiceNoticia = noticias.findIndex(
    (noticias) => noticias.id === Number(id)
  );
  if (indiceNoticia === -1) {
